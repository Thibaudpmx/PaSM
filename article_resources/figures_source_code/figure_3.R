...
k2_1edgeless <- k2_1[2:(length(k2_1)-1)]
lambda0_1edgeless <-  lambda0_1[2:(length(lambda0_1)-1)]

points <- crossing(k2_1 = k2_1edgeless, lambda0_1 = lambda0_1edgeless)
...
testabove <- self$clone(deep = T)

testbelow <- self$clone(deep = T)
...
plot6 <- ggplot()+
  ...
  # geom_segment(data = tibble(lambda0_1), aes(x = min(k2_1), xend = min(k2_1), y = min(lambda0_1), yend = max(lambda0_1)), lty = 1, size = 1.2)+
  # geom_segment(data = tibble(lambda0_1), aes(x = max(k2_1), xend = max(k2_1), y = min(lambda0_1), yend = max(lambda0_1)), lty = 1, size = 1.2)+
  # geom_segment(data = tibble(lambda0_1), aes(x = min(k2_1), xend = max(k2_1), y = min(lambda0_1), yend = min(lambda0_1)), lty = 1, size = 1.2)+
...
#
#
# plot_grid(plot1, plot2, plot3, plot4, plot5, plot6,  labels = LETTERS)
#
#
#
#
